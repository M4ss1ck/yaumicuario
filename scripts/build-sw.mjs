...
async function main() {
  ...
  // Build precache list: every file as "./<path>", except index.html, which is
  // precached as "./" instead. Cloudflare Pages answers /index.html with a 308
  // to /, so caching that URL stores a response with its redirected flag set —
  // and responding to a navigation with a redirected response is what the spec
  // forbids, which failed every navigation after the first with ERR_FAILED.
  // "/" is served 200 directly, so the entry is clean and the navigation
  // fallback below has something safe to return.
  const precache = entries.map(([p]) => (p === "index.html" ? "./" : `./${p}`));
...
self.addEventListener("install", (event) => {
  event.waitUntil(
    caches.open(CACHE)
      ...
      // Take over without waiting for the old worker to release its clients.
      // Installed to the home screen, this app's window can stay controlled for
      // days; a replacement worker sitting in "waiting" behind it means that
      // window keeps running the old worker and its cache is never purged.
...
self.addEventListener("fetch", (event) => {
  ...
  // Responding to a navigation with a redirected response is a spec violation
  // and Chrome fails the whole navigation with ERR_FAILED. Nothing we precache
  // should be redirected any more, but a host that starts redirecting a URL
  // must not be able to take the site down, so never hand one back.
  const usable = (r) => (r && !(isNavigate && r.redirected) ? r : null);
  event.respondWith(
    caches.match(event.request).then((cached) => {
      if (usable(cached)) return cached;
      // For navigations, serve the cached app shell as fallback (offline support)
      if (isNavigate) {
        return caches.match("./").then((fallback) => usable(fallback) || fetch(event.request));
      }
      return fetch(event.request).then((response) => {
        // Never cache opaque or non-200 responses into the precache; just pass through
        return response;
      }).catch(() => {
        // Navigations already returned above, so this is an uncached subresource
        // with no network: there is nothing meaningful to substitute for it.
